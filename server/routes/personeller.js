...
export const PersonelRoutes = (app) => {
  app.use("/ctrlpanel/personel", new PersonellerRouter().router);
  return app.use("/", Router({ mergeParams: true }));
};
 class PersonellerRouter {
  constructor() {
    this.db = CRUD("ctrlyms", "personeller");
    this.router = Router({ mergeParams: true });
    this.router.get("/", this.personelPage.bind(this));
    this.router.get(
      "/list",
  ...
  // Personeller
  personelPage(req, res) {
    return res.render("pages/ctrlpanel/ymsfirma/personeller.hbs", {
      title: "Personeller",
